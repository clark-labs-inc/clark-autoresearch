...
//! *injected* via the [`LanguageModel`] trait rather than bundled. the crate never
...
//! the optimizer — the crate keeps that execution behind the trait so the same
...
/// A language model the host provides. the crate never calls a provider directly;
...
pub trait Proposer {
    ...
    /// the last minibatch's traces. This is the crate's unique lever over GEPA,
...
/// the `<history>` slot the crate adds for ledger-backed reflection.
