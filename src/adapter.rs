...
//!
//! This is the architectural move that turns the crate from a state model into an
...
//! the provider, sandbox, and evaluator; the crate owns the loop body and the
//! state. Provider calls, code editing, sandboxing, and network tools stay
//! behind [`ResearchAdapter`] and never enter the crate's core.
//!
...
//! sandbox inside its engines (`gepa.oa`); the crate pushes both back to the host
...
/// architectures, configs — as a `dict[str, str]`. the crate keeps the same
/// opaque representation; the adapter and proposer interpret it, the crate does
/// not.
...
/// opaque to the crate and consumed by [`ResearchAdapter::make_reflective_dataset`];
...
pub struct EvaluationBatch {
    ...
    pub scores: Vec<f64>,
    /// Raw per-example outputs, opaque to the crate.
    #[serde(default)]
...
///
/// Implementors own the provider, sandbox, and evaluator. the crate calls only
...
///
/// the crate's gates are command strings (`cargo test`, smoke checks, safety
