...
//!
//! This is the architectural core that turns clark from a state model into an
...
//! [`ResearchAdapter`] and [`Proposer`] traits. clark owns the loop body and
//! the state; the host owns execution.
//!
//! Compare GEPA's `gepa.core.engine.GEPAEngine.run`, which bundles the
//! reflection LM and the candidate pool inside the engine. clark keeps the
    ...
    P: Proposer,
{
    ...
    while !stop.should_stop(&state.snapshot) {
        ...
        // Render the ledger dossier as reflection history (clark's unique lever:
        // reflection reads the whole history, not just the last minibatch).
        ...
        // Gate-gated acceptance in Validate mode (clark's unique lever; GEPA
